...
describe('Create the users for message exchange', () => {
  ...
  it('should return a 400 bad request response if the message type is not sent, inbox, or draft', (done) => {
    chai.request(server)
      .delete(`${messagesRoute}/multiple/`)
      .set('Authorization', `${details.rotimiToken}`)
      .send({ type: 'notNormal', ids: [900, 390] })
      .end((req, res) => {
        res.should.be.json;
        res.should.have.status(400);
        expect(res.body).to.have.own.property('error', 'type must be inbox, sent or draft');
        done();
      });
  });
  ...
  it('should successfully delete the specified messages', (done) => {
    chai.request(server)
      .delete(`${messagesRoute}/multiple`)
      .set('Authorization', `${details.nNekaToken}`)
      .send({ ids: [Number(`${details.fifthMessageId}`), Number(`${details.sixthMessageId}`)], type: 'inbox' })
      .end((req, res) => {
        res.should.have.status(200);
        res.should.be.json;
        expect(res.body).to.have.own.property('data', 'All specified messages have been deleted');
  ...
  it('should successfully delete the specified messages', (done) => {
    chai.request(server)
      .delete(`${messagesRoute}/multiple`)
      .set('Authorization', `${details.nNekaToken}`)
      .send({ ids: [Number(`${details.fifthMessageId}`), Number(`${details.sixthMessageId}`)], type: 'draft' })
      .end((req, res) => {
